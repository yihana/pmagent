...
# GoT는 ScheduleAgent 쪽 payload에서 use_got 옵션으로 켜는 방식 가정
...
# ------------------------------------------------------------
# E1: Scope 품질/효율 실험
#   - Baseline: 내부 저수준 함수(_extract_items_with_confidence)
#   - Deep: pipeline(ToT + Self-Refine)
# ------------------------------------------------------------
def experiment_E1_scope(rfps: List[str]) -> List[Dict[str, Any]]:
    ...
    for idx, rfp_text in enumerate(rfps):
        ...
        # Baseline
        t0 = time.time()
        baseline_items, _raw = asyncio.run(
            scope._extract_items_with_confidence(
                rfp_text,
        ...
        baseline_req_count = len(baseline_items.get("requirements", []))
        print(f"  Baseline: req={baseline_req_count}, time={baseline_time:.2f}s")

        # Deep (ToT + Self-Refine)
        t1 = time.time()
        deep = asyncio.run(
            scope.pipeline(
                {
...
def print_summary_e1(e1: List[Dict[str, Any]]):
    base_reqs = [r["baseline_req_count"] for r in e1]
    deep_reqs = [r["deep_req_count"] for r in e1]
    base_t = [r["baseline_time"] for r in e1]
    deep_t = [r["deep_time"] for r in e1]

    if not e1:
        print("[SUMMARY] E1: no data")
        return
...
if __name__ == "__main__":
    ...
    print_summary_e1(e1)
    visualize_e1_results(e1, results_dir)  # ⭐ 추가
    ...
    visualize_e2_results(e2, results_dir)  # ⭐ 추가
    ...
    print("\n[INFO] All experiment results saved under", results_dir)
...
def visualize_e1_results(e1_results: List[Dict[str, Any]], results_dir: Path):
    ...
    rfp_ids = [r['rfp_id'] for r in e1_results]
    baseline_reqs = [r['baseline_req_count'] for r in e1_results]
    deep_reqs = [r['deep_req_count'] for r in e1_results]
    baseline_times = [r['baseline_time'] for r in e1_results]
    deep_times = [r['deep_time'] for r in e1_results]
...
def create_summary_report(e1, e2, e3, e4, results_dir: Path):
    ...
    if e1:
        base_reqs = [r['baseline_req_count'] for r in e1]
        deep_reqs = [r['deep_req_count'] for r in e1]
        base_times = [r['baseline_time'] for r in e1]
        deep_times = [r['deep_time'] for r in e1]
    ...
    if e4:
        ...
        req_counts = [r['req_count'] for r in e4]
