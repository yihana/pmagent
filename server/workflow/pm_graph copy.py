...
async def _analyze_handler(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    출력: {ok, document_id, saved_action_items, ...} -> 출력: {ok, project_id, document_id, meeting_id, saved_action_items, ...}
    """
    project_id = payload.get("project_id")
    
    # ✅ project_id 검증 및 타입 변환
    if not project_id:
    ...
    try:
        ...
        for idx, item in enumerate(raw_items or [], start=1):
            try:
                ai = pm_models.PM_ActionItem(
                    project_id=project_id,
                    document_id=doc.id,
                    meeting_id=item.get("meeting_id") if "meeting_id" in item else None,  # ✅ Meeting이 생성되었으면 연결
        ...
        logger.info(f"[ANALYZE] Saved {saved} action items for project_id={project_id}, document_id={doc.id}, meeting_id={meeting_id}")
        
        return {
            "ok": True,
            "project_id": project_id,
            "document_id": doc.id,
            "meeting_id": meeting_id,  # ✅ Meeting ID 포함
            "doc_type": doc_type,
